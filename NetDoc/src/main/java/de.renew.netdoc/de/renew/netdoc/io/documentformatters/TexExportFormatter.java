    ...
    protected void formatImpl(DocumentPart targetPart)
        ...
        if (main == null) {
            throw new DocumentFormatException(
                            "Target document part did not contain main-tag");
        }
        if (!(main instanceof LinearDocumentPart)) {
            throw new DocumentFormatException(
                            "main-tag of target document part is not linear");
        }

        LinearDocumentPart linearMain = (LinearDocumentPart) main;

        if (!linearMain.containsPart(
                        DocumentParts.NAME_NETDOC_TARGET_PACKAGE)) {
            linearMain.addPartAt(0, new TexTargetPackagePart(
                            DocumentParts.NAME_NETDOC_TARGET_PACKAGE,
                            targetPart));
        }
        if (!linearMain.containsPart(DocumentParts.NAME_NETDOC_TARGET_NAME)) {
            linearMain.addPartAt(0, new TexTargetNamePart(
                            DocumentParts.NAME_NETDOC_TARGET_NAME, targetPart));
        }
    }
}