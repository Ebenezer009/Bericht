...
/**
 * Close-Request container manager.
 *
 * @author Christian Bracker,
 * <a href="mailto:1bracker@informatik.uni-hamburg.de">
 * 1bracker@informatik.uni-hamburg.de</a>
 */
public abstract class CloseRequestManager extends AbstractContainerManager
                implements CloseRequestListener {
     ...
     * #allowClosingImpl(CloseRequestEvent)}.</p>
    ...
    protected boolean allowClosingImpl(CloseRequestEvent event) {
        Enumeration<CloseRequestListener> listenerEnum = this
                        .getCloseRequestListeners();
    ...
    public void close(boolean forceClose) throws ContainerException {
        ...
        for (int index = 0; index < documentContainers.length; index++) {
            if (forceClose) {
                ...
                if (!this.allowClosing(new CloseRequestEvent(
                                documentContainers[index]))) {
                    return;
                }
                documentContainers[index].close(forceClose);
                if (this.getDocumentContainers()
                                .contains(documentContainers[index])) {
                    return;
                }
            }
        }
    ...
    protected void addCloseRequestListenerImpl(CloseRequestListener listenerToAdd) {
        ...
        CloseRequestListener[] newListeners = new CloseRequestListener[oldListeners.length
                        + 1];
    ...
    protected Enumeration<CloseRequestListener> getCloseRequestListenersImpl() {
        return Collections.enumeration(
                        Arrays.asList(this._closeRequestListeners));
    }
    ...
    protected void addListeners(DocumentContainer containerToObserve) {
        ...
        containerToObserve.addDocumentContainerListener(
    ...
    protected void removeListeners(DocumentContainer containerToUpdate) {
        containerToUpdate.removeDocumentChangeListener(
                        this.getDocumentListener());
        containerToUpdate.removeDocumentContainerListener(
                        this.getDocumentListener());
    }
     ...
     */
    protected void closeDocument(Document documentToClose)
                    throws ContainerException {
        DocumentContainer container = this
                        .getDocumentContainer(documentToClose);
        if (container != null) {
     ...
     */
    protected void closeDocumentContainer(DocumentContainer containerToClose)
                    throws ContainerException {
     ...
     */
    protected class DocumentListener extends DocumentContainerAdapter
         ...
         * <p>To override implement {@link
         * #documentChangedImpl(DocumentChangeEvent)}.</p>
        ...
        protected void containerClosingImpl(DocumentContainerEvent event) {
            if (CloseRequestManager.this.allowClosing(
                            new CloseRequestEvent(event.getOriginator()))) {
                CloseRequestManager.this.fireDocumentContainerEvent(event);
                try {
                    CloseRequestManager.this.closeDocumentContainer(
                                    event.getOriginator());
        ...
        protected void documentClosingImpl(DocumentContainerEvent event) {
            if (CloseRequestManager.this.allowClosing(new CloseRequestEvent(
                            event.getInvolvedDocument()))) {
                CloseRequestManager.this.fireDocumentContainerEvent(event);
                try {
                    CloseRequestManager.this
                                    .closeDocument(event.getInvolvedDocument());
                } catch (ContainerException e) {
                }
            }
        }
