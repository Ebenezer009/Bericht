...
/**
 * This class provides the ContainerManager for the Renew GUI.
 *
...
public class GUIContainerManager extends UserRequestManager {
    ...
    public GUIContainerManager(GuiPlugin renewGUIPlugin) {
        ...
        new Thread() {
            @Override
            public void run() {
                while (!GUIContainerManager.this.getGUIPlugin().isGuiPresent()) {
                    Thread.yield();
                }
                JFrame frame = GUIContainerManager.this.getGUIPlugin().getGuiFrame();
    ...
    protected Collection<DocumentContainer> getDocumentContainersImpl() {
        return Collections.unmodifiableCollection(this.getContainerMap().values());
    }
     ...
     */
    @Override
    protected DocumentContainer openDocumentImpl(DocTarget docTarget) throws ContainerException {
        // get document
     ...
     */
    @Override
    protected DocumentContainer openDocumentImpl(Document documentToOpen) throws ContainerException {
     ...
     * @de.renew.ensure (returnValue != null)
     */
    protected DocumentContainer createContainer(Document document) throws ContainerException {
    ...
    /**
     * Creates or loads the document for the specified documentation target.
     *
     * @param docTarget the documentation target specifying the document.
     * @return the document.
     * @throws ContainerException if an error occurred while creating or loading
     * the document.
     * @de.renew.require (docTarget != null)
     * @de.renew.ensure (returnValue != null)
     */
    protected Document createOrLoadDocument(DocTarget docTarget) throws ContainerException {
        if (this.getIOManager().documentPartExists(docTarget)) {
            try {
                return (Document) this.getIOManager().loadDocumentPart(docTarget);
            } catch (IOException e) {
     ...
     * @throws ContainerException Exception in NetDoc Container occurred
     * @de.renew.require (container != null)
     */
    protected void addContainer(DocumentContainer container) throws ContainerException {
        ...
        try {
            ...
            if (document.getTarget() != null) {
                JFrame documentFrame = ((Window) container).getJavaFrame();
                JFrame targetFrame = GUIDocTargets.getTargetFrame(this.getGUIPlugin(), document.getTarget());
                ...
                CloseListener closeListener = this.new CloseListener(windowListeners, document);
                TitleChangeListener titleChangeListener = this.new TitleChangeListener(document);

                targetFrame.addPropertyChangeListener("title", titleChangeListener);
                targetFrame.addWindowListener(closeListener);

                WindowConnectionListener connectionListener = new WindowConnectionListener(documentFrame, targetFrame);
        ...
        ((Window) container).getJavaFrame().addWindowListener(new WindowAdapter() {
            @Override
            public void windowOpened(WindowEvent event) {
                GUIContainerManager.this.fireDocumentContainerEvent(
                        new DocumentContainerEvent(DocumentContainerEvent.DOCUMENT_OPENED, iContainer, iDocument));
    ...
    protected void removeDocumentListeners(Document document) {
        ...
        DocumentContainer documentContainer = this.getDocumentContainer(document);
        JFrame targetFrame = GUIDocTargets.getTargetFrame(this.getGUIPlugin(), document.getTarget());

        PropertyChangeListener[] propertyListeners = targetFrame.getPropertyChangeListeners();
        for (int index = 0; index < propertyListeners.length; index++) {
            if (propertyListeners[index] instanceof TitleChangeListener) {
                targetFrame.removePropertyChangeListener(propertyListeners[index]);
            }
        }
        ...
        for (int index = 0; index < windowListeners.length; index++) {
            if (windowListeners[index] instanceof WindowConnectionListener) {
                if (((WindowConnectionListener) windowListeners[index]).getWindow1() ==
                        ((Window) documentContainer).getJavaFrame()) {
                    targetFrame.removeComponentListener((ComponentListener) windowListeners[index]);
                    targetFrame.removeWindowListener(windowListeners[index]);
                }
            } else if (windowListeners[index] instanceof CloseListener) {
                targetFrame.removeWindowListener(windowListeners[index]);
                WindowListener[] wrappedListeners = ((CloseListener) windowListeners[index]).getWrappedListeners();
    ...
    protected class GUIDocumentListener extends DocumentListener {
        ...
        protected void documentClosedImpl(DocumentContainerEvent event) {
            DocumentContainer container = GUIContainerManager.this.getDocumentContainer(event.getInvolvedDocument());

            GUIContainerManager.this.removeDocumentListeners(event.getInvolvedDocument());
            GUIContainerManager.this.getContainerMap().remove(event.getInvolvedDocument());
    ...
    protected class GUIIOManager extends SimpleIOManager {
         ...
         */
        public GUIIOManager() {
            super(new TexExportFormatter(), new DocumentInputFormatter(), new DocumentOutputFormatter(),
                    new TexDocumentParser(), GUIContainerManager.FILENAME_EXTENSION);
        }
         ...
         */
        @Override
        protected DocumentPart loadDocumentPartImpl(DocTarget correspondingTarget) throws IOException {
         ...
         */
        @Override
        protected void saveDocumentPartImpl(DocumentPart partToSave) throws IOException {
    ...
    private class CloseListener extends CloseRequestListener {
         ...
         * @de.renew.require (document != null)
         */
        public CloseListener(WindowListener[] listenersToWrap, Document document) {
        ...
        protected boolean allowClosing(WindowEvent event) {
            ...
            return (GUIContainerManager.this.getDocumentContainer(this._document) == null);
        }
        ...
        public void propertyChange(PropertyChangeEvent event) {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    ((NetDocWindow) GUIContainerManager.this.getDocumentContainer(
                            TitleChangeListener.this._document)).updateTitle();
                }
            }.start();
        }
    ...
    private class GUIFrameListener extends CloseRequestListener {
        ...
        public void fireWindowClosing(java.awt.Window closingWindow) {
            this.fireWindowClosing(new WindowEvent(closingWindow, WindowEvent.WINDOW_CLOSING));
        }
    }
}