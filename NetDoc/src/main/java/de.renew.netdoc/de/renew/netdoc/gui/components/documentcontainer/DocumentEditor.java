...
import de.renew.netdoc.gui.swing.JUndoTextArea;
import de.renew.netdoc.gui.swing.JUndoTextField;
...
/**
 * NetDoc document editor component.
 *
...
public class DocumentEditor extends SingleDocumentContainerComponent {
    ...
    public DocumentEditor(Component parent) {
        ...
        JScrollPane descriptionAreaPanel = new JScrollPane(
    ...
    protected void closeDocumentImpl(Document documentToClose,
        ...
        if (documentToClose == this.getDocument()) {
            if (forceClose) {
                ...
                this.fireDocumentContainerEvent(new DocumentContainerEvent(
                                DocumentContainerEvent.DOCUMENT_CLOSED, this,
                                documentToClose));
            } else {
                this.fireDocumentContainerEvent(new DocumentContainerEvent(
                                DocumentContainerEvent.DOCUMENT_CLOSING, this,
                                documentToClose));
            }
        }
    }
        ...
        if (this.getDocument() != null) {
            ...
            if (this.getDocument() != null) {
                throw new ContainerException(
                                "Unable to close current document");
            }
        }
        this.setDocument(documentToOpen);
        this.fireDocumentContainerEvent(new DocumentContainerEvent(
                        DocumentContainerEvent.DOCUMENT_OPENED, this,
                        documentToOpen));
    }
    ...
    protected void addEditorParts(Document document) {
        document.setPart(this.new TextDocumentPart(
                        document.getPart(DocumentParts.NAME_NETDOC_AUTHOR),
                        this.getAuthorTextField()));
        document.setPart(this.new TextDocumentPart(
                        document.getPart(DocumentParts.NAME_NETDOC_TITLE),
                        this.getTitleTextField()));
        document.setPart(this.new TextDocumentPart(
                        document.getPart(DocumentParts.NAME_NETDOC_DESCRIPTION),
    ...
    protected void removeEditorParts(Document document) {
        document.setPart(((DocumentEditor.TextDocumentPart) document
                        .getPart(DocumentParts.NAME_NETDOC_AUTHOR))
                                        .getWrappedPart());
        document.setPart(((DocumentEditor.TextDocumentPart) document
                        .getPart(DocumentParts.NAME_NETDOC_TITLE))
                                        .getWrappedPart());
        document.setPart(((DocumentEditor.TextDocumentPart) document
                        .getPart(DocumentParts.NAME_NETDOC_DESCRIPTION))
                                        .getWrappedPart());
    }
    ...
    protected void addListeners(Document document) {
        this.getAuthorTextField().getDocument().addDocumentListener(
                        (DocumentListener) document.getPart(
                                        DocumentParts.NAME_NETDOC_AUTHOR));
        this.getTitleTextField().getDocument().addDocumentListener(
                        (DocumentListener) document.getPart(
                                        DocumentParts.NAME_NETDOC_TITLE));
        this.getDescriptionArea().getDocument().addDocumentListener(
                        (DocumentListener) document.getPart(
    ...
    protected void removeListeners(Document document) {
        ...
        this.getAuthorTextField().getDocument().removeDocumentListener(
                        (DocumentListener) document.getPart(
                                        DocumentParts.NAME_NETDOC_AUTHOR));
        this.getTitleTextField().getDocument().removeDocumentListener(
                        (DocumentListener) document.getPart(
                                        DocumentParts.NAME_NETDOC_TITLE));
        this.getDescriptionArea().getDocument().removeDocumentListener(
                        (DocumentListener) document.getPart(
                                        DocumentParts.NAME_NETDOC_DESCRIPTION));
    }
    ...
    protected void updateComponents(Document document) {
        if (document != null) {
            ...
            this.getAuthorTextField().setText(
                            document.getPart(DocumentParts.NAME_NETDOC_AUTHOR)
                                            .getText());
            this.getDescriptionArea().setText(document
                            .getPart(DocumentParts.NAME_NETDOC_DESCRIPTION)
                            .getText());
        } else {
            this.getTitleTextField().setText("");
    ...
    protected ChangeListener getChangeListener() {
        if (this._changeListener == null) {
            this._changeListener = new DocumentEditor.ChangeListener();
        }
     ...
     * editing.
     */
    protected class TextDocumentPart extends DocumentPartWrapper
         ...
         */
        public TextDocumentPart(DocumentPart partToWrap,
                                JTextComponent associatedComponent) {
            super(partToWrap);
        ...
        protected Object cloneImpl() {
            return new TextDocumentPart(this.getWrappedPart(),
                            this.getAssociatedComponent());
        }
     ...
     * this editor is working on. Notfies all document change listener of this
     * editor about document changes.
     */
    protected class ChangeListener extends StructureChangeAdapter
         ...
         * </p>
         * <p>
         * To override implement {@link
         * #textChangedImpl(TextChangeEvent)}.
         * </p>
         *
        ...
        protected void textChangedImpl(TextChangeEvent event) {
            DocumentEditor.this.fireDocumentChangeEvent(new DocumentChangeEvent(
                            DocumentEditor.this.getDocument(), event));
        }
