...
/**
 * This class is a TeX document parser that can parse TeX documents from a
 * Reader and construct a corresponding document model.
 *
...
public class TexDocumentParser extends AbstractDocumentParser {
     ...
     */
    @Override
    protected DocumentPart parseFromImpl(Reader documentReader) throws IOException {
     ...
     */
    protected DocumentPart parseFrom(BufferedReader documentReader) throws IOException {
     ...
     * @de.renew.ensure (returnValue != null)
     */
    private LinearDocumentPart parseBlockDefinitionFrom(BufferedReader documentReader) throws IOException {
        ...
        if (line == null) {
            throw new DocumentParseException("block definition part not terminated");
        } else {
            LinearDocumentPart definition = new TexBlockDefinitionPart(name);

            if (name.equals(DocumentParts.NAME_NETDOC_DOCUMENT) || (parts.size() > 1)) {
                ...
                while (partIterator.hasNext()) {
                    ...
                    if (current instanceof String) {
                        if (!current.equals("")) {
                            definition.appendPart(new TextDocumentPart((String) current));
                        }
                    } else {
     ...
     * @de.renew.ensure (returnValue != null)
     */
    private DocumentPart parseDefinitionFrom(BufferedReader documentReader) throws IOException {
        documentReader.mark(TexDocumentParser.READ_AHEAD_LIMIT);
        String line = documentReader.readLine();
        ...
        TexDefinitionPart definition = new TexDefinitionPart(line.substring(1, sepIndex));
     ...
     * @de.renew.ensure (returnValue != null)
     */
    private String readToDefinitionStart(BufferedReader documentReader) throws IOException {
        StringBuffer buffer = new StringBuffer();
