...
import de.renew.netdoc.gui.swing.gui.components.documentcontainer.NetDocPanel;
...
/**
 * This class hold the Multi-Window-Mode for the NetDoc GUI window.
 *
...
public class NetDocWindow extends SingleDocumentContainerWindow {
    ...
    public NetDocWindow() {
        ...
        ((JComponent) this.getPanel().getJavaComponent()).setBorder(new EmptyBorder(4, 2, 2, 2));

        this.getPanel().addDocumentChangeListener(this.getContainerListener());
        this.getPanel().addDocumentContainerListener(this.getContainerListener());

        this.getFrame().addWindowListener(new FrameListener());
        this.getFrame().getContentPane().add(this.getPanel().getJavaComponent());
    ...
    protected class ContainerListener extends DocumentContainerAdapter implements DocumentChangeListener {
        ...
        protected void containerClosedImpl(DocumentContainerEvent event) {
            NetDocWindow.this.fireDocumentContainerEvent(
                    new DocumentContainerEvent(event.getType(), NetDocWindow.this, event.getInvolvedDocument()));
        }
